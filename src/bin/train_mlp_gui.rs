...
impl MLPTrainingGUI {
    ...
    fn start_training(&mut self) {
        ...
        self.total_epochs = 150; // harus match mlp_config.n_epochs
        ...
        thread::spawn(move || {
            ...
            // ── Phase 1: Load Data ──
            push(&format!("\n{}", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".yellow()));
            push(&format!("{}", " Phase 1/4: Loading Dataset".bold().yellow()));
            ...
            let (train_dataset, val_dataset, _) = match loader.load_all_data() {
                Ok(d) => d,
                Err(e) => { push(&format!("❌ Gagal load data: {}", e)); return; }
            };
            ...
            // ── Phase 2: Preprocess ──
            push(&format!("\n{}", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".yellow()));
            push(&format!("{}", " Phase 2/4: Preprocessing".bold().yellow()));
            ...
            // MLP: (samples, channels, timesteps) — permuted_axes([0,2,1])
            let train_mlp = train_processed.permuted_axes([0, 2, 1]);
            let val_mlp   = val_processed.permuted_axes([0, 2, 1]);
            push("   ✅ Preprocessing selesai");
            push("   📐 Ekstrak 48 fitur statistik per sample (8 sensor × 6 stat)");
            ...
            push(&format!(
                "   Arsitektur: 48 → {} → {} → 2",
                mlp_config.hidden1, mlp_config.hidden2
            ));
            push(&format!(
            ...
            let (fit_result, accuracy_curve, loss_curve) = match mlp.fit_with_curve(
                &train_mlp,
            ...
            let train_probs = mlp.predict(&train_mlp);
            ...
            let val_probs = mlp.predict(&val_mlp);
            ...
            let val_correct = val_preds.iter().zip(val_dataset.labels.iter()).filter(|(p,l)| p==l).count();
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("mlp_header")
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.add_space(20.0);
                    ui.label(egui::RichText::new("🧠 MLP Training")
                    ...
                    if pred_resp.clicked() {
                        std::thread::spawn(|| {
                            ...
                            let binary_path = std::env::current_exe()
                                .ok()
                                .and_then(|p| p.parent().map(|d| d.join("predict_mlp_gui")))
                                .unwrap_or_else(|| std::path::PathBuf::from("./target/release/predict_mlp_gui"));
            ...
            .show(ctx, |ui| {
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        ...
                        // ── Data Panel ──
                        ui.label(egui::RichText::new("📂 Data Files")
                            .size(14.0).color(egui::Color32::BLACK)
                            .family(egui::FontFamily::Name("PoppinsBold".into())));
                        ui.add_space(6.0);
                        ...
                        ui.horizontal(|ui| {
                            egui::Frame::none().fill(egui::Color32::WHITE).inner_margin(im).rounding(5.0)
                        ...
                        if self.is_training {
                            egui::Frame::none().fill(egui::Color32::WHITE).inner_margin(15.0).rounding(5.0)
                                ...
                                .show(ui, |ui| {
                                    ...
                                    egui::ScrollArea::vertical().max_height(150.0).stick_to_bottom(true)
                                        .show(ui, |ui| {
                                            for line in &log {
                                                ui.label(egui::RichText::new(line)
                                                    .size(12.0).color(egui::Color32::from_rgb(60, 60, 60))
                                                    .family(egui::FontFamily::Monospace));
                                            }
                                        });
                                    // ── Power ──
...
fn main() -> eframe::Result<()> {
    ...
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title("MLP Training"),
        ..Default::default()
    };
    eframe::run_native("MLP Training", options, Box::new(|cc| {
...
fn setup_fonts(ctx: &egui::Context) {
    ...
    if let Ok(data) = std::fs::read("assets/fonts/Poppins-Regular.ttf") {
        fonts.font_data.insert("Poppins".to_owned(), egui::FontData::from_owned(data));
        fonts.families.get_mut(&egui::FontFamily::Proportional).unwrap().insert(0, "Poppins".to_owned());
        fonts.families.insert(egui::FontFamily::Name("Poppins".into()), vec!["Poppins".to_owned()]);
    }
    if let Ok(data) = std::fs::read("assets/fonts/Poppins-Bold.ttf") {
        fonts.font_data.insert("PoppinsBold".to_owned(), egui::FontData::from_owned(data));
        fonts.families.get_mut(&egui::FontFamily::Proportional).unwrap().insert(0, "PoppinsBold".to_owned());
